...
- **범인 지목.** "초기화중" 글자가 화면에 보였다는 점이 결정적 단서다. 글자가 떴다 = 첫 페인트가 이미 일어났다 = 1단계(render-blocking)는 통과했다는 뜻이다. 만약 1단계가 걸렸다면 "초기화중" 글자조차 안 떴을 것이다. 따라서 범인은 첫 페인트 이후에 도는 **간트 초기화 시점의 `await`(2단계, dhx-icons)**다.
- **근본 대응.** 어느 쪽이 진짜 원인이든 안 걸리게 하려고 **"임계 경로의 외부 요청을 전부"** 제거한 것이 핵심이다. 범인을 특정하는 대신 위험 경로 자체를 없앴다.
- ★ **이건 브라우저 버그도 React 버그도 아니다.** "외부 CDN에 의존하게 짠 우리 코드"의 **설계 선택** 문제다. (이 로직은 기존 사내 프로젝트 코드에서 그대로 물려받았다.)
...
### ③ 같은 'CDN 의존' 패턴이 3D 모델 화면에 그대로 남음
